...
docker build -f airflow/Dockerfile.runner -t asp-airflow-runner:latest airflow   # -> runner (for retraining DAG)
...
```bash
cp .env.example .env
#   edit airflow/.env: set HOST_PROJECT_ROOT (forward slashes on Windows):
#   HOST_PROJECT_ROOT=C:/Users/nithinkarkal/Documents/GitHub/accident-severity-predictor
...
✅ **Expected:** three green tasks; a fresh model appears in the host `artifacts/` folder.
...
`version_dataset_dvc` needs DagsHub creds — see Tier 3 below. Without them it will fail (expected);
with them it runs `dvc commit` + `dvc push`.

### Tear down

```bash
docker compose -f docker-compose.airflow.yaml down      # from airflow/
...
**For the retraining DAG's `version_dataset_dvc` task:** put the same token in **`airflow/.env`**:
...
Restart Airflow (`down` then `up -d`) and re-trigger `asp_retraining`; the DVC step will now push.
...
3. In **Changes**, tick the `airflow/` files and `learning-guides/` files (Phase 8 + this doc).
